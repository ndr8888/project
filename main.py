...
def win_screen(name=False):  # окно победы, принцип тот же, что и в функции выше
    direction = [0, 0]
    a = [
...
while True:
        ...
        while level_running:
            ...
            for map_num, map_name in map_rotation:
                ...
                while level_running and life_running:
                    ...
                    if player.is_killed:
                        player.change_weapon(0)
                        score.sub(50) # вычитание счёта за смерть
                        break
                        life_running = False
    ...
    if is_won:
        sc = score.get_score()
        if score.has_name():
            direction_new, state = win_screen(name=state)
